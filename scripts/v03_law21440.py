...
def fetch_detail(item):
    try:
        r=requests.get(item['source_url'],headers=H,timeout=45); r.raise_for_status()
...
def main():
    r=requests.get(URL,headers=H,timeout=60); r.raise_for_status(); soup=BeautifulSoup(r.text,'lxml')
    items=[]; seen=set()
    ...
    with ThreadPoolExecutor(max_workers=6) as ex: rows=list(ex.map(fetch_detail,items))
    ...
    summary={'source_id':'donatarias_21440','status':'CURRENT' if len(valid) else 'FAILED','listed':len(df),'valid_rut':len(valid),'active_valid_rut':int((valid.registry_status.str.lower()=='inscrita').sum()),'failed_details':int(df.entity_id.isna().sum()),'generated_at':utc_now(),'failure_is_zero':False}
